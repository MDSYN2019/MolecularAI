...
#      - Attention-based readout (GlobalAttention) or plain pooling
#    """
#
#    def __init__(
#        self,
...
#        pooling="attn",  # "attn" | "mean" | "sum" | "max"
#        use_gru=False,  # GRU over layers for stability (AttentiveFP-style)
#        use_residual=True,
#        norm="batch",  # "batch" | "layer" | None
#        global_feat_dim = 0,
#    ):
#        super().__init__()
#        assert pooling in {"attn", "mean", "sum", "max"}
...
#
#        # ---- (A) Shared edge encoder (bond features -> hidden) ----
#        # You can keep it shallow; GINEConv will still apply its node_mlp inside.
...
#            self.norms.append(self._make_norm(norm, hidden_channels))
#
#        # ---- (C) Optional GRU cell (AttentiveFP uses GRU updates) ----
#        if use_gru:
#            self.gru = nn.GRU(hidden_channels, hidden_channels)
#
#        # ---- (D) Readout: attention gating (AttentiveFP-style) or plain pooling ----
#        if pooling == "attn":
#            # gate_nn produces a scalar gate per node; we keep it light
...
#        else:
#            # attention pooling handled separately
#            raise RuntimeError("Attention pooling uses self.readout")
#
...
#                x, h_prev = self.gru(x.unsqueeze(0), h_prev)  # [1,N,H],[1,N,H]
#                x = x.squeeze(0)
#
