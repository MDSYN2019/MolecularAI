...
def datamol_clean_cartridge() -> pd.DataFrame:
    """
    Adding additonal feature columns for the dataset
    """
    dataframe = preprocess_postgres_cartridge()
    dataframe["mol"] = dataframe["structure"].apply(
        dm.to_mol
    )  # take the structure and convert to mol
    return dataframe


def add_feature_scaffold(inital_dataframe: pd.DataFrame) -> pd.DataFrame:
    """
    A scaffold is the core molecular backbone that defines a molecule's shape, binding mode,
    and phamacophore, without the more variable substituients
    """
    pass


def add_feature_two(inital_dataframe: pd.DataFrame) -> pd.DataFrame:
    """ """
    pass
...
class EMolculeConnect:
    ...
    def _preprocess_postgres_cartridge(self):
        ...
        """
        cartridge_result = asyncio.run(execute_molecular_filter())
        [value.values() for value in cartridge_result]
